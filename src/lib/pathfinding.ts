...
function computeEdgeWeight(edge: GraphEdge, strategy: RouteStrategy): number {
  switch (strategy) {
    ...
    case 'low-carbon': {
      const stationDistance = edge.type === 'travel' ? lowCarbonConfig.stationDistanceKm : 0.5;
      const metroCarbon = stationDistance * lowCarbonConfig.metroEmissionPerKm;
      const carbonWeight = metroCarbon / 10;
      const transferPenalty = edge.type === 'transfer' ? 20 : 0;
      const timeWeight = edge.time * 0.3;
      const fareWeight = edge.fare * 2;
      return carbonWeight + transferPenalty + timeWeight + fareWeight;
    }
    default:
      return edge.time;
  }
...
function calculateMetroDistanceKm(stationCount: number): number {
  return stationCount * lowCarbonConfig.stationDistanceKm;
}

function calculateBaselineCarbonGrams(totalDistanceKm: number): number {
  return totalDistanceKm * lowCarbonConfig.carEmissionPerKm;
}

function calculateBaselineFare(totalDistanceKm: number): number {
  const carCostPerKm = 0.8;
  return Math.round(totalDistanceKm * carCostPerKm * 100) / 100;
}

function buildBikeFeederSegment(
  type: 'bike-to-station' | 'bike-from-station',
  userX: number,
  userY: number,
  station: Station,
): BikeFeederSegment | null {
  const bikeParking = station.bikeParking ?? getBikeParkingByStationId(station.id);
  ...
  return {
    type,
    bikeParkingId: bikeParking.id,
    bikeParkingName: bikeParking.name,
    distanceKm,
    bikeTimeMinutes,
...
export function findRoute(
  ...
  travelMode: TravelMode = 'metro-only',
  userOrigin?: { x: number; y: number },
  userDestination?: { x: number; y: number },
): RouteResult | null {
  ...
  return buildRouteResult(foundPath, timePeriod, travelMode, startStation, endStation, userOrigin, userDestination);
}
...
function buildRouteResult(
  ...
  const metroDistanceKm = calculateMetroDistanceKm(pathNode.stationCount);
  ...
  if (travelMode === 'bike-feeder') {
    ...
    if (toSegment || fromSegment) {
      ...
      totalFare = Math.round((totalFare + totalBikeCost) * 100) / 100;

      bikeFeeder = {
        toStation: toSegment ?? undefined,
        fromStation: fromSegment ?? undefined,
        totalBikeTime,
        totalBikeCost: Math.round(totalBikeCost * 100) / 100,
      };
    }
  }
  ...
  return {
    ...
    travelMode,
    bikeFeeder,
    carbonEmissionGrams: Math.round(totalCarbonGrams),
    carbonSavedGrams: Math.round(carbonSaved),
    moneySaved: Math.round(moneySavedValue * 100) / 100,
    baselineFare: baselineFareValue,
    baselineCarbonGrams: Math.round(baselineCarbon),
  };
}
...
export function findAllRoutes(
  ...
  const strategies: RouteStrategy[] = ['shortest-time', 'min-transfers', 'lowest-fare', 'low-carbon'];

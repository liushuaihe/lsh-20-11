...
import { MapPin, Lock, Star, Bike } from 'lucide-react';
import { stations, metroLines, getStationById, getLineById, bikeParkingStations } from '@/data/metroNetwork';
...
export default function MetroMap() {
  ...
  const [hoveredBikeParking, setHoveredBikeParking] = useState<string | null>(null);
  ...
  const highlightedBikeParkingIds = useMemo(() => {
    const ids = new Set<string>();
    if (!highlightedRoute?.bikeFeeder) return ids;
    if (highlightedRoute.bikeFeeder.toStation) {
      ids.add(highlightedRoute.bikeFeeder.toStation.bikeParkingId);
    }
    if (highlightedRoute.bikeFeeder.fromStation) {
      ids.add(highlightedRoute.bikeFeeder.fromStation.bikeParkingId);
    }
    return ids;
  }, [highlightedRoute]);
  ...
  const getBikeParkingAvailability = (available: number, total: number) => {
  ...
  const renderBikeParking = (bp: typeof bikeParkingStations[number]) => {
    const availability = getBikeParkingAvailability(bp.availableSpots, bp.totalSpots);
    const isOnPath = highlightedBikeParkingIds.has(bp.id);
    const isHovered = hoveredBikeParking === bp.id;
    ...
    return (
      <g
        key={bp.id}
        className="cursor-pointer"
        onMouseEnter={() => setHoveredBikeParking(bp.id)}
        onMouseLeave={() => setHoveredBikeParking(null)}
      >
        <rect
          x={bp.x - 9}
          y={bp.y - 9}
          ...
          className={cn(
            'transition-all duration-200',
            (isHovered || isOnPath) ? 'scale-125' : '',
          )}
          style={{ transformOrigin: `${bp.x}px ${bp.y}px` }}
        />
        <text
          x={bp.x}
          y={bp.y + 4}
        ...
        {(isHovered || isOnPath) && (
          <g>
            ...
            <text
              ...
              textAnchor="middle"
              fontSize="10"
              fill={availability === 'high' ? '#86efac' : availability === 'medium' ? '#fcd34d' : '#fca5a5'}
            >
              车位 {bp.availableSpots}/{bp.totalSpots} · ¥{bp.unitPricePerHour}/小时
            </text>
          </g>
        )}
      </g>
  ...
  return (
    <div className="relative w-full h-full bg-gradient-to-br from-slate-50 to-slate-100 rounded-2xl overflow-hidden border border-slate-200 shadow-inner">
      ...
      <div className="absolute bottom-3 left-3 z-10 flex flex-col gap-1.5 bg-white/90 backdrop-blur rounded-xl p-3 border border-slate-200 shadow-sm max-h-[45%] overflow-y-auto">
        ...
        <div className="mt-2 pt-2 border-t border-slate-200 space-y-1">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-600">
            <span className="inline-block w-3 h-3 rounded bg-teal-500" />
            车位充足
          </div>
          <div className="flex items-center gap-2 text-xs text-slate-600">
      ...
      >
        ...
        {bikeParkingStations.map(renderBikeParking)}
